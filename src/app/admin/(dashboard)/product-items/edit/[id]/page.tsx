...
interface ProductCategory {
  ...
  name: LocaleField;
  productId: number;
  product?: { id: number; name: LocaleField };
}
...
export default function EditProductItemPage() {
  ...
  const fetchItem = useCallback(async () => {
    setLoadingItem(true);
    try {
      const res = await authFetch(`/api/product-items/${itemId}`);
      const item = await res.json();

      setName(item.name || emptyLocale());
      setDescription(item.description || emptyLocale());
      setColor(item.color || emptyLocale());
      setVolume(item.volume || emptyLocale());
      setComposition(item.composition || emptyLocale());
      setViticulture(item.viticulture || emptyLocale());
      setAged(item.aged || emptyLocale());

      setAlcohol(item.alcohol || 0);
      setFruitTones(item.fruitTones || 0);
      setTannins(item.tannins || 0);
      setSweetness(item.sweetness || 0);
      setBody(item.body || 0);
      setTemperature(item.temperature || "");
      setGlass(item.glass || "wine");
      setIsPopular(item.isPopular || false);

      setProductCategoryId(item.productCategoryId ? String(item.productCategoryId) : "");
      setCompanyId(item.companyId ? String(item.companyId) : "");
      setFoodIds(item.foods?.map((f: Food) => f.id) || []);

      // Set product from category
      if (item.productCategory?.product?.id) {
        setSelectedProductId(String(item.productCategory.product.id));
      }

      // Files
      setImageId(item.imageId || null);
      setImagePreview(item.image?.url || "");
      setVinificationId(item.vinificationId || null);
      setVinificationPreview(item.vinification?.url || "");
      setAwardIds(item.awards?.map((a: FileObj) => a.id) || []);
      setAwardPreviews(item.awards?.map((a: FileObj) => a.url) || []);
      setImageIds(item.images?.map((i: FileObj) => i.id) || []);
      setImagePreviews(item.images?.map((i: FileObj) => i.url) || []);
    } catch {
      alert("სასმელი ვერ მოიძებნა");
      router.push("/admin/product-items");
    } finally {
      setLoadingItem(false);
    }
  }, [authFetch, itemId, router]);
  ...
  }, [ready, fetchReferenceData, fetchItem]);

  useEffect(() => {
    if (selectedProductId) fetchCategories();
  }, [selectedProductId, fetchCategories]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await authFetch(`/api/product-items/${itemId}`, {
        method: "PUT",
  ...
  if (loadingItem) {
    return (
      <div className="flex items-center justify-center py-20">
        <p className="text-gray-400 text-lg">იტვირთება...</p>
      </div>
    );
  }
